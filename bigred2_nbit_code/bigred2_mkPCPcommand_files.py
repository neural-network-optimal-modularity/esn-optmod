    ...
    for i, q1 in enumerate(list_q1):
        for j, q2 in enumerate(list_q2):
            for k, sampleID in enumerate(xrange(experimental_parameters['num_reservoir_samplings'])):
                experimental_parameters['nbit_task_parameters'][experimental_parameters['q1']] = q1 
                experimental_parameters['nbit_task_parameters'][experimental_parameters['q2']] = q2 
                experimental_parameters['nbit_task_parameters']['temp_dir_ID'] = experimental_parameters['command_prefix'] + \
...
def write_qsub_script(parameters):
    ...
    file_text += "aprun -n " + str(parameters['num_threads']) + " pcp " + parameters['nbit_task_parameters']['full_path'] + \
        parameters['command_prefix'] + "_command_file.txt\n"
    file_text += "aprun -n 1 python bigred2_nbit_result_consolidation.py " + \
        parameters['command_prefix'] + " " + parameters['nbit_task_parameters']['full_path']
...
def setup_parameter_files(parameters):
    ...
    parameter_file_names = write_command_file(parameters['worker_file'], parameters['command_prefix'], 
        len(parallel_input_chunks), parameters['nbit_task_parameters']['full_path'])
    ...
    write_output_filenamelist(parameters['command_prefix'], len(parameter_file_names), 
        parameters['nbit_task_parameters']['full_path'])
    utilities.save_object({'parameters': parameters, 'q1_list': parameters['q1_list'], 'q2_list': parameters['q2_list']}, 
        parameters['nbit_task_parameters']['full_path'] + parameters['command_prefix'] + "_paramfile.pyobj")