...
def generate_unweighted_two_community_graph(N, mu, avg_degree):
    """
...
def two_com_graph_uniform(N, mu, avg_degree, lower_bound=0, upper_bound=1):
...
def two_com_graph_gamma(N, mu, avg_degree, EE_W=30000.0, negative_weights=False):
...
if __name__ == '__main__':
    ...
    import CircuitChecker
    import time
    start_time = time.time()
    graph = two_com_graph_uniform(250, 0.0, 7, -1, 1)
    print("--- %s seconds ---" % (time.time() - start_time))
    nx.write_gexf(graph, "test.gexf")
    network_checker = CircuitChecker.SliceGraphChecker(graph, graph, \
        community_key="community", prefix="test")
    network_checker.SummaryPlot()
